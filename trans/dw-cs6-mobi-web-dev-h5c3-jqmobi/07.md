...
导航至**窗口** | **jQueryMobile样本**查看面板。单击页面中的任何元素，然后单击一个样本，将该配色方案分配给该元素。顺便说一下，这可以在实时视图切换到打开状态时工作。
...
1.  导航至**文件** | **新建**，创建新的 jQueryMobile页面。
...
6.  导航至**插入** | **jQuery Mobile** | **页面**插入新页面元素。出现 **jQueryMobile页面**对话框，预设提示添加**第五页**。保持选中复选框选项，以便在页面上包含页眉和页脚。
