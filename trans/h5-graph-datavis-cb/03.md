...
![Building a bar chart from scratch](graphics/3707OT_03_01.jpg)
...
![Spreading data in a scatter chart](graphics/3707OT_03_02.jpg)
...
4.  Our last function is to render the data points and to do that we create the `createDots` function:
    ...
    这里我们为每个点计算出相同的细节——包括`y`位置和`x`位置——然后我们画一个矩形。让我们现在测试我们的应用程序！
...
![Building line charts](graphics/3707OT_03_03.jpg)
...
![There's more...](graphics/3707OT_03_04.jpg)
...
![Creating the flying brick chart (waterfall chart)](graphics/3707OT_03_05.jpg)
...
![Building a candlestick chart (stock chart)](graphics/3707OT_03_06.jpg)
...
之后，我们将为这个矩形创建一个填充，并根据`closeYOffset`变量的值设置样式颜色。在这个阶段，我们有一个正在运行的应用程序，尽管它可以使用一些更多的调整来使它工作得更好。
...
开始了。我们将`isCandle`布尔变量的默认值设置为`false`。如果我们再次运行我们的应用程序，我们会发现它以新的格式呈现。要改变这一点，我们只需要在调用`addStock`函数时提供第三个参数`true`:

![Adding other render options to our stock chart](graphics/3707OT_03_07.jpg)
