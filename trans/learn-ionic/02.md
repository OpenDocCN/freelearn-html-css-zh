...
我们将在[第 6 章](06.html#2SG6I0-9757c8e51afd47e0a7a9ced32db749b8)、*Ionic原生*中更多的处理科尔多瓦插件。
...
Ionic有自己的命令行界面来搭建、开发和部署Ionic应用。在开始使用Ionic命令行界面之前，我们需要设置几个软件。

# Ionic 3

在本书发行时，最新版本的 ion 是 3。我整理了另一章，叫做《Ionic 3》(第 11 章)，你可以参考它来了解更多关于Ionic 3 及其变化的信息。

此外，请注意，本书中的例子在使用Ionic 3 时仍然有效。可能很少有句法和结构上的变化，但总的主旨应该保持不变。

# 软件设置

现在，我们将设置顺利开发和运行一个Ionic应用所需的所有软件。
...
`npm`是一个**节点包管理器**，我们将使用它来下载我们的Ionic项目的各种依赖项。
...
Git 是一个免费的开源分布式版本控制系统，旨在以速度和效率处理从小型到超大型的所有项目。在我们的例子中，我们将使用一个名为 Bower 的包管理器，它使用 Git 来下载所需的库。此外，Ionic命令行界面使用 Git 下载项目模板。
...
最后，为了完成Ionic 2 的设置，我们将安装Ionic和科尔多瓦命令行界面。Ionic命令行界面是科尔多瓦命令行界面的包装，具有一些附加功能。
...
现在我们已经完成了软件设置，我们将搭建一些Ionic应用。

ion 有三个主要模板，我们可以使用它们快速开始开发应用:

*   空白:这是一个只有一页的空白Ionic项目
*   选项卡:这是一个使用Ionic选项卡构建的示例应用
*   侧菜单:这是一个示例应用，是为使用侧菜单驱动的导航而构建的

为了理解脚手架的基础知识，我们将从空白模板开始。

为了保持我们的学习过程干净，我们将创建一个文件夹结构来处理Ionic项目。创建名为`chapter2`的文件夹。
...
参考[附录](12.html#5AMKM0-9757c8e51afd47e0a7a9ced32db749b8)、附加主题和提示，了解更多关于Ionic启动任务的信息。
...
如果我们运行`ionic serve`并且端口`8100`已经被占用，Ionic将在`8101`上启动该应用。

我们还可以使用以下命令在任何其他端口上为Ionic应用提供服务:
...
一旦Ionic应用启动，通过在 Mac 上按*命令* + *选项* + *I* 和在 Windows/Linux 上按*Ctrl*+*Shift*+*I*打开开发者工具。然后点击顶部一行中的最后一个图标，靠近关闭按钮，如下图所示:
...
到目前为止，我们已经搭建了一个空白的Ionic应用，并在浏览器中启动了它。现在，我们将走过脚手架工程结构。
...
*   `src`:这是所有开发发生的文件夹。应用源代码将放在这里。如果你是从Ionic 1 到Ionic 2，这是你会注意到的第一个变化。对我来说，这是对文件夹结构的一个非常好的升级，因为它将开发代码与部署代码分开。
...
*   `www`:这个文件夹包含构建Ionic代码，写在`src`文件夹里面。这个文件夹中的所有代码都是为了放在网络视图中。
...
如前所述，这个文件夹由我们的Ionic应用、HTML、CSS 和 JS 代码组成。如果打开`src`文件夹，会发现如下文件结构:
...
*   `theme folder`:该文件夹由`variables.scss`组成，覆盖会改变Ionic组件的观感。
*   `index.html`:这是一切开始的地方。

我们的空白模板之旅到此结束。在搭建下一个模板之前，让我们快速浏览一下`src/app/app.component.ts`文件。

如您所见，我们正在创建一个新的应用/根组件。`@Component`装饰器需要一个`template`或`templateUrl`属性来正确加载Ionic 2 应用。作为模板的一部分，我们添加了`ion-nav`组件。

在类定义里面，我们已经声明了一个`rootPage`并分配给了主页，在构造函数里面，我们有平台就绪回调，当平台就绪的时候会调用。

这是一个非常简单和基本的Ionic应用。到目前为止，你一定已经完成了与网络相关的 Angular 代码。但是当你在处理Ionic的时候，你也会处理与设备特性相关的脚本。ion 为我们提供服务，让这些事情以更有组织的方式发生。

# 搭建标签模板

为了对Ionic命令行界面和项目结构有一个好的感觉，我们还将构建另外两个入门模板。首先，我们将搭建标签模板。
...
选项卡位于页面底部。我们将在[第 3 章](03.html#1T1400-9757c8e51afd47e0a7a9ced32db749b8)、*Ionic组件与导航*、[第 4 章](04.html#2BASE0-9757c8e51afd47e0a7a9ced32db749b8)、*Ionic装饰与服务*中详细介绍定制。

如果你回到`example2`文件夹，分析项目结构，除了`src` / `pages`文件夹的内容，一切都是一样的。

这次，您将在 pages 文件夹中看到四个文件夹。选项卡文件夹由选项卡定义组成，关于、联系人和个人文件夹由每个选项卡的定义组成。

现在，您可以很好地了解Ionic如何与 Angular 集成，以及所有组件如何携手并进。当我们处理更多的几个Ionic，这个结构会更有意义。
...
在本章中，我们获得了一些关于移动混合架构的知识。我们还了解了混合应用的工作原理。我们看到了科尔多瓦如何将 HTML、CSS 和 JS 代码拼接到一个本地应用的 WebView 中执行。然后我们安装了必要的软件，在本地开发Ionic应用。我们使用Ionic命令行界面搭建了一个空白模板，并分析了项目结构。后来，我们搭建了另外两个模板，并观察了它们的区别。
...
在下一章*Ionic组件和导航*中，我们将了解Ionic组件以及如何构建一个简单的两页应用并在它们之间导航。这将有助于我们使用Ionic应用编程接口构建有趣的用户界面和多页面应用。