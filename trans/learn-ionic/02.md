...
离子 2 是一个漂亮的开源前端软件开发工具包，用于开发带有 HTML5 的混合移动应用。ion 提供移动优化的 HTML、CSS 和 JS 组件，以及用于构建高度交互应用的手势和工具。
...
爱奥尼亚有自己的命令行界面来搭建、开发和部署爱奥尼亚应用。在开始使用离子命令行界面之前，我们需要设置几个软件。
...
现在我们已经完成了软件设置，我们将搭建一些离子应用。

ion 有三个主要模板，我们可以使用它们快速开始开发应用:

*   空白:这是一个只有一页的空白离子项目
*   选项卡:这是一个使用离子选项卡构建的示例应用
...
一旦离子应用启动，通过在 Mac 上按*命令* + *选项* + *I* 和在 Windows/Linux 上按*Ctrl*+*Shift*+*I*打开开发者工具。然后点击顶部一行中的最后一个图标，靠近关闭按钮，如下图所示:
...
到目前为止，我们已经搭建了一个空白的离子应用，并在浏览器中启动了它。现在，我们将走过脚手架工程结构。
...
*   `src`:这是所有开发发生的文件夹。应用源代码将放在这里。如果你是从离子 1 到离子 2，这是你会注意到的第一个变化。对我来说，这是对文件夹结构的一个非常好的升级，因为它将开发代码与部署代码分开。
...
如前所述，这个文件夹由我们的离子应用、HTML、CSS 和 JS 代码组成。如果打开`src`文件夹，会发现如下文件结构:
...
如您所见，我们正在创建一个新的应用/根组件。`@Component`装饰器需要一个`template`或`templateUrl`属性来正确加载离子 2 应用。作为模板的一部分，我们添加了`ion-nav`组件。
...
在本章中，我们获得了一些关于移动混合架构的知识。我们还了解了混合应用的工作原理。我们看到了科尔多瓦如何将 HTML、CSS 和 JS 代码拼接到一个本地应用的 WebView 中执行。然后我们安装了必要的软件，在本地开发离子应用。我们使用离子命令行界面搭建了一个空白模板，并分析了项目结构。后来，我们搭建了另外两个模板，并观察了它们的区别。
...
在下一章*离子组件和导航*中，我们将了解离子组件以及如何构建一个简单的两页应用并在它们之间导航。这将有助于我们使用离子应用编程接口构建有趣的用户界面和多页面应用。