...
在最后一章中，我们介绍了一些Ionic组件，使用这些组件可以轻松构建经典的移动混合应用。在这一章，我们将与Ionic 2 装饰和服务。整个 ion 2 生态系统分为两部分:组件和服务 API。组件包括按钮、卡片和列表，正如我们在上一章中看到的，服务 API 包括平台、`config`、`NavController`、`Storage`等。
...
在我们开始使用Ionic内置装饰器之前，我们将很快了解什么是装饰器，以及它们如何让我们的生活变得轻松。
...
*   Ionic模块或`NgModule`装饰器
*   组件装饰器

# Ionic模块

Ionic模块或`NgModule`装饰器启动Ionic应用。如果我们打开任何一个现有的Ionic项目并查找`src/app/app.module.ts`文件，我们将看到以下内容:
...
这是我们引导Ionic应用的地方。也可以使用`IonicModule`上的`forRoot`来配置该应用。`forRoot`同时负责提供和配置服务。
...
首先，我们将搭建一个空白的Ionic应用。创建一个名为`chapter4`的新文件夹，并在该文件夹中打开一个新的命令提示符/终端，然后运行以下命令:
...
现在，我们将使用Ionic命令行界面的生成命令生成一个名为关于的新页面。运行以下命令:
...
平台服务返回关于当前平台的可用信息。Ionic全新改进的平台服务有更多信息，帮助我们根据设备类型定制应用。
...
您应该能够在支持科尔多瓦的浏览器中看到Ionic应用的启动，现在输出应该如下所示:
...
因此，使用存储，我们可以轻松地在我们的Ionic应用中开始处理数据持久性，而不用担心底层实现。
...
在这一章中，我们已经介绍了Ionic的两个主要装饰者。然后，我们浏览了配置和平台服务，了解了如何根据平台和配置定制应用。在此之后，我们已经通过了Ionic存储应用编程接口。参见[第 11 章](11.html#523VK0-9757c8e51afd47e0a7a9ced32db749b8)、*Ionic 3* ，了解全新的`IonicPage`指令和`IonicPage`模块。

在下一章中，我们将研究Ionic应用的主题。