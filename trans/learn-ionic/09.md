...
在这一章中，我们将看看测试一个使用科尔多瓦(和离子 2)构建的移动混合应用。测试可以在多层中进行，首先是单元测试，然后是端到端测试，最后将应用部署到实际设备上并执行测试。在本章中，我们将使用我们在[第 8 章](08.html#3UQQQ0-9757c8e51afd47e0a7a9ced32db749b8)、*ion 2 迁移指南*中构建的 Ionic 2 Todo 应用，并对其执行以下测试:
...
截至本章撰写之日，由爱奥尼亚命令行界面搭建的应用不包括任何测试设置。因此，我们需要自己添加所需的测试设置。
