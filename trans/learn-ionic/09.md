...
在这一章中，我们将看看测试一个使用科尔多瓦(和Ionic 2)构建的移动混合应用。测试可以在多层中进行，首先是单元测试，然后是端到端测试，最后将应用部署到实际设备上并执行测试。在本章中，我们将使用我们在[第 8 章](08.html#3UQQQ0-9757c8e51afd47e0a7a9ced32db749b8)、*ion 2 迁移指南*中构建的 Ionic 2 Todo 应用，并对其执行以下测试:
...
截至本章撰写之日，由Ionic命令行界面搭建的应用不包括任何测试设置。因此，我们需要自己添加所需的测试设置。
...
*   当本地通知被触发时(这就是我们在Ionic原生插件上进行单元测试的方式)
...
在写这一章的三天前，Ionic团队发布了一篇博文，表明他们将支持单元和端到端测试，这将是Ionic脚手架项目本身的一部分。更多信息可以在这里找到:[http://blog.ionic.io/basic-unit-testing-in-ionic/](http://blog.ionic.io/basic-unit-testing-in-ionic/)。

这个项目是基于几个非常有价值的贡献者在Ionic 2 测试领域，正如博客文章中提到的。截止到今天，*漂移/Ionic-单元测试-示例*([https://github.com/driftyco/ionic-unit-testing-example](https://github.com/driftyco/ionic-unit-testing-example))回购没有完整的实现，仅支持单元测试。
