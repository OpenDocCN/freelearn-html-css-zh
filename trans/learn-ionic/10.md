...
在这一章中，我们将看看为您的Ionic应用生成安装程序的三种方法。一个使用 PhoneGap 构建服务，第二个使用 Cordova CLI，最后，第三个使用 Ionic package 服务。我们将为安卓和 iOS 操作系统生成安装程序。我们将在本章中讨论以下主题:
...
这将负责将您的图像上传到Ionic云，根据需要调整它们的大小，并将它们保存回资源文件夹。
...
使用Ionic云服务生成安装程序非常简单。首先，我们通过运行以下程序将我们的应用上传到我们的Ionic账户:
...
在这一章中，我们看到了如何发布和管理一个Ionic应用。我们看到了如何使用 PhoneGap 构建服务、使用 Cordova CLI 以及最后使用 Ionic Package 来生成安装程序。

在下一章中，我们将看看Ionic 3 以及Ionic 2 和Ionic 3 之间的主要区别。

请注意，到目前为止，我们学习的几乎所有概念仍然适用于Ionic 3。