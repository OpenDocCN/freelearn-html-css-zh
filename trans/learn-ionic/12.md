...
这本书的主要目的是让读者尽可能多地了解爱奥尼亚。因此，从第 1 章到第 11 章，我遵循了一个增量方法，从 Cordova 的基础知识到用 Angular Ionic 和 Cordova 构建一个应用。我们非常专注于学习最低限度的离子。
...
如果您使用的是离子云服务，那么您在云上创建的每个项目都将被分配一个应用标识(有关更多信息，请参考本章中的离子云部分)。该应用标识将位于`ionic.config.json`文件中，位于项目的根目录。
...
使用“离子状态”命令，您可以管理离子项目的状态。让我们假设您正在添加一些插件和平台来测试您的离子应用中的一些东西。但是如果失败了，你就不想用这些了。在这种情况下，您可以使用保存和恢复命令。
...
离子提供了一种在浏览器、模拟器和设备中运行离子应用的简单方法。这三个命令都有很多有用的选项。
...
您可以在[https://apps.ionic.io/apps](https://apps.ionic.io/apps)创建和管理您的爱奥尼亚应用。在前面的命令中，我们提到的应用标识是我们使用[https://apps.ionic.io/apps](https://apps.ionic.io/apps)界面创建新应用时生成的应用标识。
...
要使用这些服务，我们需要首先构建一个离子应用，然后通过运行以下命令将该应用添加到离子云中:
